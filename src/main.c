...
void logStatus(char *log, HAL_StatusTypeDef status);
...
int main(void) {
  ...
  while (1) {
    ...
    // DMAstatus =
    //     HAL_DMA_PollForTransfer(&hdma_spi2_rx, HAL_DMA_FULL_TRANSFER, 10);
    // if (DMAstatus == HAL_ERROR) {
    //   // no transfer so start again
    // HAL_I2S_Receive_DMA(&hi2s2, &sampleBuffer[0], 48 * 2);
    // }
